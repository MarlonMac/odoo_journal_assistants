...
class ExpenseAssistant(models.Model):
    ...
    payable_account_id = fields.Many2one('account.account', string='Cuenta por Pagar', domain="[('deprecated', '=', False)]", help="Cuenta de pasivo que se usará para registrar la deuda.")
    responsible_employee_ids = fields.Many2many('hr.employee', string='Empleados Responsables')
    category_id = fields.Many2one('expense.category', string='Categoría de Gasto', required=True, states={'posted': [('readonly', True)], 'cancelled': [('readonly', True)]})
    ...
    payment_journal_id = fields.Many2one('account.journal', string='Diario de Pago (Empresa)', domain="[('type', 'in', ('bank', 'cash'))]", help="Diario de la empresa utilizado para realizar el pago directo.")
